//
//  Styles.swift
//  Tagsy
//
//  Created by Trevor Pennington on 8/19/20.
//  Copyright © 2020 Trevor Pennington. All rights reserved.
//

import Foundation
import UIKit

public var serifFont = "Iowan Old Style"
public var sansFont = "GillSans"

public var titleStyle = UIFont(name: serifFont, size: 24)
public var inputStyle = UIFont(name: serifFont, size: 14)
public var tagStyle = UIFont(name: sansFont, size: 14)
public var sansTitleStyle = UIFont(name: sansFont, size: 16)
public var serifTitleStyle = UIFont(name: serifFont, size: 16)
public var largeSansStyle = UIFont(name: sansFont, size: 24)

public let background = UIColor.systemGray6
public let textHex = UIColor.systemGray
